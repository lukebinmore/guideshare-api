# [**Go Back**](https://github.com/lukebinmore/guideshare-api)

# **Table Of Contents**
- [**Go Back**](#go-back)
- [**Table Of Contents**](#table-of-contents)
- [**Database Scheme**](#database-scheme)
  - [**Profile Table**](#profile-table)
  - [**Post Table**](#post-table)
  - [**Category Table**](#category-table)
  - [**Votes Table**](#votes-table)
  - [**Comments Table**](#comments-table)
  - [**Contact Forms Table**](#contact-forms-table)

***

# **Database Scheme**

Below are the planned fields for each table in the database managed by the API. These may change depending on site requirements as development progresses. These have been created utilizing [Diagrams.net](https://app.diagrams.net/).

## **Profile Table**

![Profile Table](/docs/databasetables/profile-table.png)

***

## **Post Table**

![Post Table](/docs/databasetables/post-table.png)

***

## **Category Table**

![Category Table](/docs/databasetables/category-table.png)

***

## **Votes Table**

![Votes Table](/docs/databasetables/votes-table.png)

***

## **Comments Table**

![Comments Table](/docs/databasetables/comment-table.png)

***

## **Contact Forms Table**

![Contact Forms Table](/docs/databasetables/contact-form-table.png)

***